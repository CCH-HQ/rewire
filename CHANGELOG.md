...
- Aligned adapter recipes with current client configuration contracts: Claude gateway bearer tokens; an isolated Codex Responses provider/profile that preserves model selection and normalizes bare origins to `/v1`; OpenCode's preferred `opencode.jsonc`, native OpenAI/Anthropic providers with managed model catalogs, protocol-versioned SDK roots, custom-provider SDK fallback, restricted file substitution, and qualified selected model; Hermes named-provider fields, structured model selection, and `.env`; and OpenClaw's `models.providers` nesting, selected primary model, file SecretRef, and per-model transport/base URL overrides for mixed catalogs.
...
- Aligned Hermes with its current keyed `providers` resolver and structured `model` schema, platform-specific Windows directory rules, and ownership-scoped alias migration without dropping operator fields; moved the adapter into its own module.
- Made Hermes preserve the selected model family's wire protocol: Claude uses its native Messages transport and request root, OpenAI uses Responses, and compatible fallbacks use OpenAI Chat Completions instead of sending every model to one schema.
- Split Claude Code, Codex, OpenCode, Hermes, and OpenClaw adapters into dedicated modules with separate adapter tests, leaving `clients.rs` as the registry and shared recipe policy.
- Added ordered Models endpoint candidates for known Anthropic-compatible request prefixes, preserving the supplied path first and falling back only after HTTP 404/405.
...
- Added a credential-isolated Docker installer E2E harness that builds the release archive, serves and verifies it over HTTP, configures all five clients under a temporary Home, proves an idempotent second plan, probes the live Models endpoint, and scans logs, assets, installation files, and transaction state for token leakage.
- Added modular real-client runtime E2E runners for pinned Claude Code, Codex, OpenCode, Hermes Agent, and OpenClaw releases. They read only Rewire's isolated configuration, perform live model calls, verify fixed responses and native completion records, and scan all captured runtime artifacts for API-token leakage.
