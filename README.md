...
Running `rewire` without arguments opens the workflow only when both standard input and standard output are terminals. Configuration invocations may also provide any subset of `--baseurl`, a token, `--client`, model, protocol, and display-name inputs; on a terminal, Rewire preserves supplied values and asks only the applicable `configure` questions that remain unanswered. A root configuration invocation first prints its numbered plan and asks for explicit confirmation; `--yes` is required for unattended apply. Pipes, automation, and `--non-interactive` fail fast on missing required inputs while retaining the normal inferred defaults for omitted optional values. The previous `rewire tui` spelling remains an alias for `rewire configure`.
